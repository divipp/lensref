...
newReference :: forall m a . (Monad m, Applicative m) => a -> RefCreatorT m (RefHandler (RefCreatorT m) a)
...
instance (Monad m, Applicative m) => RefClass (RefHandler (RefCreatorT m)) where
